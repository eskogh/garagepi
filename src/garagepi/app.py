import os, time, threading, signal, atexit
...
def _publish_availability(state: str):
    if _mq:
        _mq.publish(TOPIC_AVAIL, state, qos=1, retain=True)

def _publish_state_if_changed(force: bool=False):
    ...
    def on_connect(client, userdata, flags, reason_code, properties=None):
        try:
            ...
            hamq.publish_discovery(
                client, DISCOVERY_PREFIX, NODE_ID,
                TOPIC_AVAIL, TOPIC_COVER_STATE, TOPIC_COVER_SET,
                TOPIC_CM_STATE, TOPIC_CM_SET
            )
            # Subscriptions
            client.subscribe(TOPIC_COVER_SET, qos=1)
            client.subscribe(TOPIC_CM_SET, qos=1)
            # Online + current states
